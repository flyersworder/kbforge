...
- **The package is a uv workspace member at `packages/okfquery/`.** Distribution
  name `okfquery`, import name `okfquery`. `[tool.uv.workspace] members = ["packages/*"]`
  already discovers it; the root still needs three edits (Task 1).
...
[dependency-groups]
dev = ["kbforge-mcp", "okfquery"]
```

`[tool.uv.sources]` needs **no** entry for okfquery: nothing in the workspace
depends on `okfquery` as a package, and okfquery's own dev group resolves
`kbforge` through the `kbforge = { workspace = true }` line already there.
