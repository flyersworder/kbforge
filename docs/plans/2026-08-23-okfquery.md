...
- **The package is a uv workspace member at `packages/okfquery/`.** Distribution
  ...
  still needs three edits (Task 1). The distribution is namespaced because PyPI
...
`[tool.uv.sources]` needs **no** entry for `kbforge-okfquery`: nothing in the
workspace depends on it as a package, and its own dev group resolves `kbforge`
through the `kbforge = { workspace = true }` line already there. Note the root
`[dependency-groups]` entry is the *distribution* name (`kbforge-okfquery`),
while every import and the console script remain `okfquery`.
...
Counting for the invariant: eleven `.md` files, two of them reserved-and-fenceless
(`index.md`, `log.md`), so `count(*) FROM concepts` is **nine**. `claims/index.md`
is among the nine — that is the assertion protecting the reserved rule.
...
def test_no_silent_drops_invariant():
    ...
    assert len(files) == 11
    con = load(MESSY)
    assert con.execute("select count(*) from concepts").fetchone()[0] == 9
...
`test_no_silent_drops_invariant` (9 becomes 8). Record both failure messages.
