...
- **Every test file's basename must be unique across the whole repo.** pytest runs
  in its default `prepend` import mode, and `tests/` and `packages/okfquery/tests/`
  both lack `__init__.py`, so two files sharing a basename collide at collection.
  `--import-mode=importlib` is NOT an option: `packages/kbforge-mcp/tests` does
  `from tests.fake_server import ...`, which only resolves under `prepend`. Adding
  `__init__.py` is not one either — `kbforge-mcp/tests` already owns the bare
  package name `tests`. Hence `test_okfquery_cli.py` rather than `test_cli.py`,
  which the root `tests/` already has.
...
└── tests/
    ...
    ├── test_okfquery_cli.py      exit codes and formats (basename must be repo-unique)
...
- Create: `packages/okfquery/tests/test_okfquery_cli.py`
...
`packages/okfquery/tests/test_okfquery_cli.py`:
...
Run: `uv run pytest packages/okfquery/tests/test_okfquery_cli.py -q`
...
**Files:**
- Modify: `packages/okfquery/README.md` — Task 1 created a minimal stub because
  hatchling validates that the `readme` field's target exists. **Replace its
  contents entirely**; do not append to it.
