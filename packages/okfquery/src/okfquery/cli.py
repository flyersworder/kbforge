...
from okfquery.index import (
    HandWrittenIndexError,
    is_current,
    is_generated,
    write_index,
)
...
def _index_in(bundle: Path, args: argparse.Namespace) -> int:
    if args.check:
        if not is_generated(bundle):
            # "stale; run okfquery index" would be advice that cannot work:
            # the write refuses this file too.
            print(
                f"{bundle / 'index.md'} was not generated by okfquery; "
                "run `okfquery index --force` to replace it",
                file=sys.stderr,
            )
            return 1
